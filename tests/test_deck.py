...
def test_many_draws():
    game_state = create_test_state(draw_pile=create_deck(), discard_pile=[CardId.NUMBER_1])
    for _ in range(NUM_CARDS_IN_FULL_DECK):
        draw_card(game_state, random.Random())
    assert(len(game_state.draw_pile) == 1)
    assert(len(game_state.discard_pile) == 0)
    assert(CardId.NUMBER_1 in game_state.draw_pile)
...
def test_count_unseen_updates_on_deck_shuffle():
    game_state = create_test_state(draw_pile=[CardId.NUMBER_2], discard_pile=[CardId.NUMBER_0, CardId.FLIP_THREE, CardId.TIMES_2])
